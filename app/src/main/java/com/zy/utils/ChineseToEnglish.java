...
public class ChineseToEnglish {
    ...
    private static String[] fx = new String[]{
    ...
    private static ChineseToEnglish chineseSpelling = new ChineseToEnglish();

    public static ChineseToEnglish getInstance() {
        return chineseSpelling;
    }

    private String[] diffFx(String name) {
        String[] names = new String[2];
        if (name.length() > 1) {
            names[0] = name.substring(0, 1);
            names[1] = name.substring(1, name.length());
            if (name.length() >= 3) {
                for (int i = 0; i < fx.length; i++) {
                    if (name.contains(fx[i])) {
                        names[0] = name.substring(0, 2);
                        names[1] = name.substring(2, name.length());
                    }
                }
            }
        } else {
...
//			System.out.println("分解姓："+names[0]);
//			System.out.println("分解名："+names[1]);

        return names;
    }

    public String[] transN(String name) {
        String[] fN = new String[2];
        if (name.length() < 1) {
            fN[0] = "";
            fN[1] = "";
        } else {
            String[] oN = diffFx(name);
            for (int i = 0; i < oN.length; i++) {
                if ("".equals(oN[i])) {
                    fN[i] = "";
                } else {
                    fN[i] = convertAll(oN[i]);
                }
            }
        }
        return fN;
